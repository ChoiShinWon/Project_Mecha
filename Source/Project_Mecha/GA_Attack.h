...
//
// Description:
// - Melee attack ability class.
// - Uses montage and notifies in Blueprint to control attack timing.
// - Detects targets via sphere sweep in PerformAttackTrace, applies GAS damage or engine damage.
...
//
// Description:
// - Melee attack ability. On activation, triggers Blueprint event to start montage,
//   and calls PerformAttackTrace from notify to damage enemies.
UCLASS()
...
public:
    ...
    // Attack trace function called from Blueprint (montage notify, etc.).
    // - Detects enemies ahead via sphere sweep and applies damage.
...
protected:
    ...
    // Ability end logic: Cleans up tags if needed.
    ...
    // Event called when montage is triggered in Blueprint.
...
protected:
    ...
    // 공격 범위 (전방 거리).
    // Attack range (forward distance).
    UPROPERTY(EditDefaultsOnly, Category = "Attack")
    float AttackRange = 250.f;

    // 공격 반경 (구체 스윕 반경).
    // Attack radius (sphere sweep radius).
    UPROPERTY(EditDefaultsOnly, Category = "Attack")
